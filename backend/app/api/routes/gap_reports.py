...
def create_gap_report(
    ...
    Uses GEMINI_API_KEY_GAP_MAPPER (requirement extraction + embeddings) —
    but only on a cache miss for this exact resume+JD content pair. Reuses
    module 5.1's stored parse; the resume is never re-parsed here.
    """
    try:
