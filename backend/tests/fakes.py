...
class FakeMatchScorer:
    """Stand-in for ml.inference.MatchScorer (same surface the gap mapper
    uses: ``predict`` plus ``extractor.extract_skills``).

    Configured with a fixed score/confidence so tests choose deterministically
    between the confident-ML path and the escalate-to-Gemini path.
    """

    def __init__(self, match_score, confidence, resume_skills, jd_skills):
        self.winner = "FakeModel"
        self._match_score = float(match_score)
        self._confidence = float(confidence)
        self._resume_skills = set(resume_skills)
        self._jd_skills = set(jd_skills)
        self._jd_text = None
        self.predict_calls = 0
        self.extractor = self  # extract_skills lives on the same object

    def predict(self, resume_text, jd_text):
        self.predict_calls += 1
        self._jd_text = jd_text
        label = (
            "Strong Fit" if self._match_score >= 70
            else "Moderate Fit" if self._match_score >= 40
            else "Weak Fit"
        )
        return {
    ...
    def extract_skills(self, text):
        return self._jd_skills if text == self._jd_text else self._resume_skills
