...
export default function UploadPanel({ resume, uploading, error, onUpload, onRetry }) {
  ...
  return (
    <section aria-labelledby="upload-heading">
      <div
        className={`rounded-2xl border-2 border-dashed bg-ink-800 p-6 shadow-card transition-colors sm:p-8 ${
          dragging ? "border-gold-500 bg-gold-500/10" : "border-ink-700"
        }`}
        onDragOver={(event) => {
      ...
      >
        {uploading ? (
          <div className="mx-auto max-w-sm py-2">
            <h2 id="upload-heading" className="mb-4 font-display text-lg font-semibold text-paper-50">
              Reading your resume
            </h2>
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            <span className="flex h-12 w-12 items-center justify-center rounded-full bg-ink-700 text-paper-200/70">
            ...
            <h2 id="upload-heading" className="font-display text-lg font-semibold text-paper-50">
              {resume ? "Replace your resume" : "Upload your resume"}
            </h2>
            <p className="max-w-sm text-sm text-paper-200/70">
              {resume
            ...
            <button
              ...
              className="mt-1 rounded-full bg-gold-500 px-6 py-2.5 text-sm font-semibold text-ink-950 transition-all hover:bg-gold-400 active:scale-95"
            >
              Browse files
            </button>
            <input
      ...
      {resume?.cached && !uploading && (
        <p role="status" className="mt-3 rounded-lg bg-emerald-900/40 px-3 py-2 text-sm text-emerald-300">
